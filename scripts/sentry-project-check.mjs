...
function getConfig() {
  ...
  const allowPermissionSkip = isEnabledBooleanValue(
...
/** @param {string | undefined} value */
function isEnabledBooleanValue(value) {
  if (!value) {
    return false;
  }

  return ENABLED_BOOLEAN_VALUES.has(value.toLowerCase());
}
...
async function main() {
  ...
  const projectResponse = await sentryApi(config, projectPath, {
    optionalStatuses: config.allowPermissionSkip ? [403] : [],
  });

  if (projectResponse === undefined) {
    console.warn(
      `${prefix} Sentry project access could not be verified with this token; skipping strict validation.`,
    );
    return;
  }
  ...
  const ownershipResponse = await sentryApi(
    ...
    {
      optionalStatuses: config.strictOwnership
        ? config.allowPermissionSkip
          ? [403]
          : []
        : [403, 404],
    },
